...
function cepatlakoo_migration_admin_notice(){
    ...
    if (!file_exists( WP_PLUGIN_DIR . '/redux-framework/redux-framework.php') && strpos($_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']  , 'page=tgmpa-install-plugins') && !strpos($_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']  , 'tgmpa-install=install-plugin')){
        ...
        if( $old_option ){
            ...
            $new_data = array(
               ...
               'cepatlakoo_backtotop_bg_color' => array(
                ...
                'cepatlakoo_woocommerce_striketrough_price_color' => array(
                        'regular' => cL_exist( $proc_data, 'cepatlakoo_woocommerce_striketrough_price_color' ),
                ),
