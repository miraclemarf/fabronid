	...
	public function get_default_args() {
		return apply_filters(
			...
			array(
				...
				'data_delegate'       => null,
				'save_callback'       => null,
				'string-repeat-field' => esc_html__( 'Add New', 'landingpress-wp' ),
				'string-delete-field' => esc_html__( 'Remove Field', 'landingpress-wp' ),
			),
			get_class( $this )
		);
	}
	...
	public function html() {
		...
		<div class="cmb-file-wrap" <?php echo 'data-type="' . esc_attr( $data_type ) . '"'; ?>>
			...
			<button class="button cmb-file-upload <?php echo esc_attr( $this->get_value() ) ? 'hidden' : '' ?>">
				<?php esc_html_e( 'Add File', 'landingpress-wp' ); ?>
			</button>

			<div class="cmb-file-holder type-file <?php echo esc_attr( $this->get_value() ) ? '' : 'hidden'; ?>">

				<?php if ( $this->get_value() ) : ?>

					<?php if ( isset( $src ) ) echo '<img src="' . esc_url( $src ) . '" alt="" />'; ?>
			...
			<button class="cmb-remove-file button <?php echo esc_attr( $this->get_value() ) ? '' : 'hidden'; ?>">
				<?php esc_html_e( 'Remove', 'landingpress-wp' ); ?>
			</button>
	...
	public function html() {
		...
		<div class="cmb-file-wrap" style="<?php echo esc_attr( $styles ); ?>" data-type="<?php echo esc_attr( $data_type ); ?>">
			...
			<button class="button cmb-file-upload <?php echo esc_attr( $this->get_value() ) ? 'hidden' : '' ?>" data-nonce="<?php echo wp_create_nonce( 'cmb-file-upload-nonce' ); ?>">
				<?php esc_html_e( 'Add Image', 'landingpress-wp' ); ?>
			</button>

			<div class="cmb-file-holder type-img <?php echo esc_attr( $this->get_value() ) ? '' : 'hidden'; ?>" data-crop="<?php echo (bool) $size['crop']; ?>">
...
class CMB_Date_Timestamp_Field extends CMB_Field {
	...
	public function html() { ?>
		...
		<input <?php $this->id_attr(); ?> <?php $this->boolean_attr(); ?> <?php $this->class_attr( 'cmb_text_small cmb_datepicker' ); ?> type="text" <?php $this->name_attr(); ?>  value="<?php echo esc_attr( $this->value ) ? esc_attr( date( 'm\/d\/Y', $this->value ) ) : '' ?>" />
...
class CMB_Datetime_Timestamp_Field extends CMB_Field {
	...
	public function html() { ?>
		...
		<input <?php $this->id_attr('date'); ?> <?php $this->boolean_attr(); ?> <?php $this->class_attr( 'cmb_text_small cmb_datepicker' ); ?> type="text" <?php $this->name_attr( '[date]' ); ?>  value="<?php echo esc_attr( $this->value ) ? esc_attr( date( 'm\/d\/Y', $this->value ) ) : '' ?>" />
		<input <?php $this->id_attr('time'); ?> <?php $this->boolean_attr(); ?> <?php $this->class_attr( 'cmb_text_small cmb_timepicker' ); ?> type="text" <?php $this->name_attr( '[time]' ); ?> value="<?php echo esc_attr( $this->value ) ? esc_attr( date( 'h:i A', $this->value ) ) : '' ?>" />
	...
	public function html() {
		...
		if ( $this->is_placeholder() ) 	{
			...
			<script>
				...
				cmb_wysiwyg_editors.<?php echo esc_attr( $field_id ); ?> = '<?php echo esc_attr( $editor ); ?>';
			</script>
...
class CMB_Select extends CMB_Field {
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( 'select2', trailingslashit( CMB_URL ) . 'js/vendor/select2/js/select2.full.min.js', array( 'jquery' ) );
	...
	public function enqueue_styles() {
		...
		wp_enqueue_style( 'select2', trailingslashit( CMB_URL ) . 'js/vendor/select2/css/select2.min.css' );
	}
	...
	public function output_field() {
		...
		$none = is_string( $this->args['allow_none'] ) ? $this->args['allow_none'] : esc_html__( 'None', 'landingpress-wp' );
		...
		>
			...
			<?php if ( $this->args['allow_none'] ) : ?>
				<option value=""><?php echo esc_attr( $none ); ?></option>
			<?php endif; ?>
	...
	public function output_script() {
		...
		$options = wp_parse_args( $this->args['select2_options'], array(
			'placeholder' => esc_html__( 'Type to search', 'landingpress-wp' ),
			'allowClear'  => true,
		) );
...
class CMB_Group_Field extends CMB_Field {
	...
	public function get_default_args() {
		return array_merge(
			...
			array(
				'fields'              => array(),
				'string-repeat-field' => esc_html__( 'Add New Group', 'landingpress-wp' ),
				'string-delete-field' => esc_html__( 'Remove Group', 'landingpress-wp' ),
			)
		);
	}
	...
	public function get_default_args() {
		return array_merge(
			...
			array(
				...
				'default_zoom'                => '8',
				'string-marker-title'         => esc_html__( 'Drag to set the exact location', 'landingpress-wp' ),
				'string-gmaps-api-not-loaded' => esc_html__( 'Google Maps API not loaded.', 'landingpress-wp' ),
			)
		);
	}
