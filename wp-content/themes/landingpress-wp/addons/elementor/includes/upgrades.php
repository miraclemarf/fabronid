	...
	private static function _upgrade_v0110() {
		...
		foreach ( $post_ids as $post_id ) {
			...
			$data = Plugin::$instance->db->iterate_data( $data, function( $element ) {
				...
				if ( 'button' === $element['widgetType'] || 'button_bbm' === $element['widgetType'] || 'button_line' === $element['widgetType'] || 'button_sms' === $element['widgetType'] || 'button_tel' === $element['widgetType'] || 'optin_2steps' === $element['widgetType'] ) {
					$size_to_replace = [
						'small' => 'xs',
						'medium' => 'sm',
						'large' => 'md',
