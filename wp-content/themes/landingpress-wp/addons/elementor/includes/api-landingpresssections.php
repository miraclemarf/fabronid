 ...
 * @since 1.0.0
 */
class Api_Landingpresssections {
	 ...
	 */
	public static $api_info_url = 'http://api3.landingpress.net/sections/info/';
	 ...
	 */
	private static $api_feedback_url = 'http://api3.landingpress.net/sections/feedback/';
	 ...
	 */
	private static $api_get_template_content_url = 'http://api3.landingpress.net/sections/content/';
	private static $api_get_template_thumbnail_url = 'http://api3.landingpress.net/sections/thumbnail/';
	...
	private static function _get_info_data( $force_update = false ) {
		...
		$cache_key = 'landingpresssections_info_api_data_' . ELEMENTOR_VERSION;
		...
		if ( $force_update || false === $info_data ) {
			...
			if ( isset( $info_data['templates'] ) ) {
				update_option( 'landingpresssections_info_templates_data', $info_data['templates'], 'no' );

				unset( $info_data['templates'] );
			}

			if ( isset( $info_data['feed'] ) ) {
				update_option( 'landingpresssections_remote_info_feed_data', $info_data['feed'], 'no' );
	...
	public static function get_templates_data( $force_update = false ) {
		...
		$templates = get_option( 'landingpresssections_info_templates_data' );
	...
	public static function ajax_reset_api_data() {
		check_ajax_referer( 'landingpresssections_reset_library', '_nonce' );
	...
	public static function init() {
		add_action( 'wp_ajax_landingpresssections_reset_library', [ __CLASS__, 'ajax_reset_api_data' ] );
	}
